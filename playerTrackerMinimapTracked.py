...
import deep_sort_realtime.deep_sort.kalman_filter as KF
...
def imageToPitchHomography(coord,c):
    pitchPoint = hImagesToPitch[c] @ coord
...
def pitchToMinimapHomography(coord):
    minimapPoint = hPitchToMinimap @ coord
...
def fullHomography(x,y,c):
    # Image coordinate to pitch coordinate
    pitchPoint = imageToPitchHomography(np.asarray([[x],[y],[1]]),c)

    # Pitch coordinate to minimap coordinate
    minimapPoint = pitchToMinimapHomography(pitchPoint)
...
# Initialising tracks and Kalman filter
tracks = []
kf = KF.KalmanFilter()
beginTracking = 148
    ...
    if (rets[0] == True) and (rets[1] == True) and (rets[2] == True):
        ...
        for c in range (cameras):
            ...
            for i in range (len(boundingBoxes)):
                ...
                pitchCoord = imageToPitchHomography(np.asarray([[midpoint],[y2],[1]]),c)[:2].T[0].tolist()
                detectionsPitchPositions.append(pitchCoord)
                # print(detectionsPitchPositions[i])
                # cv.waitKey(0)
            
            allDetectionsPitchPositions.append(detectionsPitchPositions)
        ...
        # Tracking
        if (frameNumber>=beginTracking):
            # Finding and combining correspondences between frames
            links = []
            # Iteration over each combination of cameras to find detection links
            for i in range (cameras-1):
                for j in range (i+1,cameras):
                    if (len(allDetectionsPitchPositions[i]) > 0) and (len(allDetectionsPitchPositions[j]) > 0):
                        # Hungarian algorithm to match detections between cameras
                        xA = np.asarray(allDetectionsPitchPositions[i])
                        xB = np.asarray(allDetectionsPitchPositions[j])
                        distances = dis.cdist(xA, xB, metric='euclidean')
                        alteredDistances = (distances.T * np.invert(np.all(distances > 3, axis = 1))).T
                        alteredDistances *= np.invert(np.all(distances > 3, axis = 0))
                        alteredDistances = np.where(alteredDistances == 0, 1000+alteredDistances, alteredDistances)
                        rowIndexes, colIndexes = linear_sum_assignment(alteredDistances)
                        trueIndexes = []
                        for row,col in zip(rowIndexes, colIndexes):
                            if not (alteredDistances[row][col] == 1000):
                                trueIndexes.append((row,col))
                        links.append(trueIndexes)
            # print(links)

            # Combining detections based on links
            # This is only specific to having 3 cameras where 0 & 2 don't overlap (not generalised)
            for i in range (len(links)):
                for link in links[i]:
                    if (i==0):
                        oldPos1 = allDetectionsPitchPositions[0][link[0]]
                        oldPos2 = allDetectionsPitchPositions[1][link[1]]
                        x = (oldPos1[0] + oldPos2[0])/2
                        y = (oldPos1[1] + oldPos2[1])/2
                        allDetectionsPitchPositions[0][link[0]] = [x,y]
                        allDetectionsPitchPositions[1][link[1]] = None
                    # elif (i==1): # Don't have to do this because there will never be a link here
                    elif (i==2):
                        oldPos1 = allDetectionsPitchPositions[1][link[0]]
                        oldPos2 = allDetectionsPitchPositions[2][link[1]]
                        x = (oldPos1[0] + oldPos2[0])/2
                        y = (oldPos1[1] + oldPos2[1])/2
                        allDetectionsPitchPositions[1][link[0]] = None
                        allDetectionsPitchPositions[2][link[1]] = [x,y]

            # Removing merged detections
            filteredPositions = []
            a = 1
            h = 1
            for i in range (cameras):
                for j in range (len(allDetectionsPitchPositions[i])):
                    if (allDetectionsPitchPositions[i][j] != None):
                        x = allDetectionsPitchPositions[i][j][0]
                        y = allDetectionsPitchPositions[i][j][1]
                        filteredPositions.append([x, y, a, h])

                        # Draws a point on the minimap
                        minimapPoint = pitchToMinimapHomography(np.asarray([[allDetectionsPitchPositions[i][j][0]], [allDetectionsPitchPositions[i][j][1]], [1]]))
                        cv.circle(minimap, (int(minimapPoint[0][0]),int(minimapPoint[1][0])), 3, (255,0,255), -1)

            # print(allDetectionsPitchPositions)
            # print("-----")
            filteredPositions = np.asarray(filteredPositions)
            # print(filteredPositions)
            # print(filteredPositions.shape)

            # Kalman filter predictions and distance from detections
            for i in range (len(tracks)):
                tracks[i][0], tracks[i][1] = kf.predict(tracks[i][0], tracks[i][1])

            # Hungarian algorithm to match tracks and detections
            distances = []
            unlinkedDetections = [i for i in range (len(filteredPositions))]
            if (len(tracks)>0) and (len(filteredPositions)>0):#
                for i in range (len(tracks)):
                    # gating = kf.gating_distance(tracks[i][0], tracks[i][1], filteredPositions, True)
                    distances.append(kf.gating_distance(tracks[i][0], tracks[i][1], filteredPositions, True).tolist())
                    # print("distances:",i)
                    # print(gating)
                    # print(gating.shape)
                # print(distances)
                distances = np.asarray(distances)
                alteredDistances = (distances.T * np.invert(np.all(distances > 1e+03, axis = 1))).T
                alteredDistances *= np.invert(np.all(distances > 1e+03, axis = 0))
                alteredDistances = np.where(alteredDistances == 0, 0, alteredDistances)
                # print(distances)
                ...
                newColIndexes = []
                for row,col in zip(rowIndexes, colIndexes):
                    if not (alteredDistances[row][col] == (0)):
                        newRowIndexes.append(row)
                        newColIndexes.append(col)
                # print(newRowIndexes)
                # print(newColIndexes)
                # print("=====")
                for row,col in zip(newRowIndexes, newColIndexes):
                    tracks[row][0], tracks[row][1] = kf.update(tracks[row][0], tracks[row][1], filteredPositions[col]) 
                    unlinkedDetections.remove(col)

            # Create new tracks
            if (len(tracks)<25):
                # Force second goalkeeper and other linesman track position
                if (frameNumber==beginTracking):
                    mean, cov = kf.initiate(np.asarray([5.5,34,1,1]))
                    tracks.append([mean, cov])
                    mean, cov = kf.initiate(np.asarray([26,0,1,1]))
                    tracks.append([mean, cov])
                # Create new tracks for unlinked detections
                for i in range (len(unlinkedDetections)):
                    if (len(tracks)>=25):
                        break
                    mean, cov = kf.initiate(filteredPositions[unlinkedDetections[i]])
                    tracks.append([mean, cov])

            # Finds all track positions and draws them on the minimap
            for i,track in enumerate(tracks):
                ...
                # Draws a point on the minimap
                minimapPoint = pitchToMinimapHomography(np.asarray([[x], [y], [1]]))
                cv.circle(minimap, (int(minimapPoint[0][0]),int(minimapPoint[1][0])), 3, (0,255,255), -1)
                cv.putText(minimap, str(i), (int(minimapPoint[0][0]),int(minimapPoint[1][0])), cv.FONT_HERSHEY_SIMPLEX, 0.5 , (255,255,0))
        ...
        cv.imshow('Minimap', minimap)
        # if (frameNumber==90 or frameNumber==270):
        if (frameNumber>=beginTracking):
            cv.waitKey(0)
