...
def fullHomography(x,y,c):
    ...
    # return [int(minimapPoint[0][0]), int(minimapPoint[1][0])]
...
while (cap[0].isOpened() and cap[1].isOpened() and cap[2].isOpened()):
    ...
    if (rets[0] == True) and (rets[1] == True) and (rets[2] == True):
        ...
        if (frameNumber>=90):
            links = []
            # Iteration over each combination of cameras
            for i in range (cameras-1):
                for j in range (i+1,cameras):
                    if (len(allDetectionsPitchPositions[i]) > 0) and (len(allDetectionsPitchPositions[j]) > 0):
                        ...
                        # Might need to up 3 based on frames around 500-600ish where separate detections
                        ...
                        alteredDistances = alteredDistances.tolist()
            ...
            # print(links)
            # This is only specific to having 3 cameras where 0 & 2 don't overlap (not general)
