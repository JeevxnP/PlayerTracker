...
while (cap[0].isOpened() and cap[1].isOpened() and cap[2].isOpened()):
    ...
    if (rets[0] == True) and (rets[1] == True) and (rets[2] == True):
        ...
        for c in range (cameras):
            ...
            kps1 = ()
            kps2 = ()
            pitchMask = np.zeros(frameCopy.shape[:2], dtype='uint8')
            ...
            for i in range (len(boundingBoxes)):
                ...
                # if (frameNumber>=90):
                # if (frameNumber>=90) and (i<=15) and (i>=13):
                    # croppedImage = frameCopy[y1:y2, x1:x2]
                    # cv.rectangle(pitchMask,(x1,y1),(x2,y2),255,-1)
                    # cv.drawContours(pitchMask, usedContours, -1, 255, -1)
                    # masked = cv.bitwise_and(frameCopy, frameCopy, mask=pitchMask)
                    # cv.imshow("Rectangular Mask", masked)
                    # cv.imshow("Framecopy", frameCopy)
                    # cv.waitKey(0)

                    # if (c==1):
                        # Detect features from the image
                        # keypoints1, descriptors1 = sift.detectAndCompute(frames[c], None)
                        # kps1 = sift.detect(frames[c], None)
                        # img1=cv.drawKeypoints(frames[c],kps1,None)
                        # img2=cv.drawKeypoints(masked,kps1,None)
                        # cv.imshow('img1', img1)
                        # cv.imshow('img2', img2)
                        # cv.waitKey(0)
                        
                    # elif(c==2):
                    #     kps2 = sift.detect(frames[c], None)
                        # img1=cv.drawKeypoints(frames[c],kps2,None)
                        # img2=cv.drawKeypoints(masked,kps2,None)
                        # cv.imshow('img1', img1)
                        # cv.imshow('img2', img2)
                        # cv.waitKey(0)
                        # # keypoints2, descriptors2 = sift.detectAndCompute(frames[c], None)

            if (frameNumber>=270):
                cv.drawContours(pitchMask, usedContours, -1, 255, -1)
                masked = cv.bitwise_and(frameCopy, frameCopy, mask=pitchMask)
                # cv.imshow('pitchMask', pitchMask)
                # cv.imshow("Rectangular Mask", masked)
                # cv.waitKey(0)
                if (c==1):
                    keypoints1, descriptors1 = sift.detectAndCompute(masked, None)
                    # kps1, des1 = sift.compute(frames[c],kps1)
                    # img1=cv.drawKeypoints(frames[c],kps1,None)
                    # cv.imshow('img1', img1)
                    # print(kps1)
                    # print(des1)
                    ...
                    keypoints2, descriptors2 = sift.detectAndCompute(masked, None)
                    # kps2, des2 = sift.compute(frames[c],kps2)
                    # img2=cv.drawKeypoints(frames[c],kps2,None)
                    # cv.imshow('img2', img2)
                    # print(kps2)
                    # print(des2)
                    # print("----------")
                    ...
                    # print(matches[15].distance)
                    # print(matches[20].distance)
                    # print(matches[30].distance)
                    
                    matchedImage = cv.drawMatches(frames[1],keypoints1,frames[2],keypoints2,matches[:50],None,flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
